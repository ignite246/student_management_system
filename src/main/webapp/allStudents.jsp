					...
					<body>
						...
						<section id="all_student_data">
							<% try { Connection conn=DBConnection.getConnection(); String
								query="select * from sms_students order by student_id asc" ; PreparedStatement
								ps=conn.prepareStatement(query, ResultSet.TYPE_SCROLL_SENSITIVE,
								ResultSet.CONCUR_UPDATABLE); ResultSet rs=ps.executeQuery(); //System.out.print("RS next
								: "+rs.next());
		%>
		<div class=" container">
								<div class="row mt-4">

									<% if (rs.next()) { rs.beforeFirst(); int count=0; while (rs.next()) { int
										st_id=rs.getInt("student_id"); String fName=rs.getString("firstname"); String
										lName=rs.getString("lastname"); String phone=rs.getString("phone"); String
										degree=rs.getString("degree"); String username=rs.getString("username"); String
										password=rs.getString("password"); int attendance=rs.getInt("attendance");
										String createdOn=rs.getString("createdOn"); String
										lastUpdatedOn=rs.getString("lastUpdatedOn"); %>
												...
												<div class="card-footer">
													<div class="container text-center">
														<a href="updateStudent.jsp?st_id=<%=st_id%>"><button
																class="btn btn-md btn-warning myShadow">Update</button></a>
														<a href="DeleteStudentServlet?st_id=<%=st_id%>"><button
